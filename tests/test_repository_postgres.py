    ...
    def test_project_backfill_and_member_permissions(self):
        ...
        with patch.dict(os.environ, database_env):
            ...
            try:
                ...
                self.assertIsNone(db.create_project_invite(personal["id"], viewer_id, "viewer"))
                invite = db.create_project_invite(personal["id"], owner_id, "viewer")
                self.assertEqual(len(invite["token"]), 32)
                db.c.execute("SELECT token_hash FROM project_invites WHERE id = %s", (invite["id"],))
                self.assertNotEqual(db.c.fetchone()[0], invite["token"])
                self.assertIsNone(db.consume_project_invite(invite["token"], owner_id))
                self.assertEqual(db.consume_project_invite(invite["token"], viewer_id)["role"], "viewer")
                self.assertIsNone(db.consume_project_invite(invite["token"], manager_id))
                revoked = db.create_project_invite(personal["id"], owner_id, "manager")
                self.assertTrue(db.revoke_project_invite(revoked["id"], personal["id"], owner_id))
                self.assertIsNone(db.consume_project_invite(revoked["token"], manager_id))
                expired = db.create_project_invite(personal["id"], owner_id, "manager")
                db.c.execute("UPDATE project_invites SET expires_at = %s WHERE id = %s",
                             (datetime.utcnow() - timedelta(seconds=1), expired["id"]))
                db.conn.commit()
                self.assertIsNone(db.consume_project_invite(expired["token"], manager_id))
                ...
                self.assertFalse(db.change_project_member_role(personal["id"], viewer_id, viewer_id, "manager"))
                self.assertTrue(db.change_project_member_role(personal["id"], owner_id, viewer_id, "manager"))
                self.assertEqual(db.get_site_role(site_id, viewer_id), "manager")
                self.assertTrue(db.change_project_member_role(personal["id"], owner_id, viewer_id, "viewer"))
